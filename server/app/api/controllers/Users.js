 ...
 create: function(req, res, next) {
   ...
   checkIfUserExists(req.body.email).then(
      (val)=>{
         if(!val){
            console.log(val);
            userModel.create({ name: req.body.name, email: req.body.email, password: req.body.password ,age:req.body.age,contactNumber:req.body.contactNumber}, function (err, result) {
               if (err)
                next(err);
               else{
                  sendMailOnRegister(result);
                  res.json({code:1,status: "success", message: "User added successfully!!!", data: result});
               }
             });
         }
         else
     ...
     if (err) {
      next(err);
         ...
         if(bcrypt.compareSync(req.body.password, userInfo.password)) {
          ...
          printUsers();
          res.json({code:1,status:"success", message: "user found!!!", data:{user: userInfo, token:token}});
         }else{
